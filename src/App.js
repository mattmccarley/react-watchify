...
import QuicklistButton from './components/Header/QuicklistButton';
import SiteTitle from './components/Header/SiteTitle';

import Body from './components/Body/Body';

import SearchResults from './components/Body/SearchResults/SearchResults';
import MovieSearchResults from './components/Body/SearchResults/MovieSearchResults';
...
function App() {
  ...
  const onSearchInputChange = (searchInputValue) => {
    setSearchInput(searchInputValue);

    if (searchInput !== '') {
      tmdb.search.getMovie({
        "query": encodeURI(searchInputValue)
      },
      (data) => {
        let parsedData = JSON.parse(data);
        setMovieSearchResults(parsedData.results.slice(0,12));
      },
      (err) => {
        console.error(err);
      });
